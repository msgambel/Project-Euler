...
- (void)initialize; {
  ...
  self.hint = @"Start with the product of the first 5 numbers in the chain, and when moving through the chain, divide by the first number, and multiply by the next number in the chain. What happens when you find a 0?";
  self.link = @"http://stackoverflow.com/questions/7141850/how-to-iterate-through-a-strings-characters";
...
- (void)computeAnswer; {
  ...
  // Here, we simply search through all of the sub-strings of the 1000 digit
  // number and (using a helper method) compute the product. We do make a slight
  // optimization, noting that if we come acrross a 0, we can skip the next 4
  // numbers, as their product will always be 0. We have a struct call
  // "MovingProduct" that helps with the computation. We then store the largest
  // product found.
...
- (void)computeAnswerByBruteForce; {
  ...
  // Here, we simply search through all of the sub-strings of the 1000 digit
  // number and computes the product. We then store the largest product found.
